...
$(document).on('change', '#select-plan-type', function () {
...
}).on('click', '#btn-generate', async function () {
    ...
    await dotnetInstance.invokeMethodAsync('GenerateSO');
    ...
    let soQuantity = +$('#txt-so-quantity').val();
    let sampleSize = +$('#txt-so-sample-size').val();

    if (soQuantity < sampleSize) {
        $('#txt-so-sample-size').val(soQuantity);
    }
});

export function setDocNum(docNum, quantity, planTypes) {
    console.log('start setDocNum');
    $('#txt-sales-order-no').val(docNum);
    $('#txt-so-quantity').val(quantity);

    $("#select-plan-type").empty();

    $.each(planTypes, function (index, option) {
        $("#select-plan-type").append(new Option(option));
    });
}
