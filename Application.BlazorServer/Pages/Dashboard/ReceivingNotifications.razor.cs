...
public partial class ReceivingNotifications : ComponentBase
{
	...
	async Task UpdateSalesOrder(DashboardNotificationViewModel @event)
	{
		await InvokeAsync(() =>
		{
			if (Convert.ToDateTime(@event.DispatchDate == "" ? DateTime.MinValue.ToString() : @event.IrradiationDate).Date == DateTime.Today)
			{
				...
				{
					if (target.Status != @event.Status && @event.Status == "Receiving")
					{
	...
	{
		if (await _jSRuntime.InvokeAsync<bool>("confirm", $"Confirm SO No. {SO.DocNum} for Receiving?", "Proceed", "Return"))
		{
			//string status = _pallet.OngoingStatus ?? "";

			//Update to SAP
			if (await _dashboardNotificationService.UpdateStatusSO(SO, "For Receiving - Ready"))
			{
				_itemList.Where(x => x.DocNum == SO.DocNum).FirstOrDefault().Status = "For Receiving - Ready";
				SO.Status = "For Receiving - Ready";
